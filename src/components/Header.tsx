...
export default function Header({
  ...
  return (
    <div className="relative bg-black">
      ...
      <header 
        className={cn(
          "w-full z-40 transition-[transform,opacity] duration-150 ease-in-out",
          !mounted
            ? "opacity-0 bg-transparent py-3 sm:py-6 relative"
            : cn(
                "opacity-100",
                isSticky
      ...
      >
        <motion.div
          ...
          className={cn(
            ...
            "transition-[background-color,box-shadow,border-radius,padding,border-color] duration-300 ease-out",
            isSticky
            ...
            {navLinks.map((link) => {
              ...
              return (
                <Link
                  key={link.name}
                  href={link.href}
                  className={`px-4 py-2 text-sm font-semibold transition-all relative rounded-full ${isActive ? "text-[#22c55e]" : "text-gray-700 hover:bg-gray-100/80"
                    }`}
                >
                  {link.name}
                  {isActive && (
              ...
              {navLinks.map((link) => {
                ...
                return (
                  <Link
                    ...
                    className={`flex w-full items-center px-4 py-3 text-sm font-medium rounded-xl text-left ${isActive ? "text-[#22c55e] font-semibold" : "text-gray-900 hover:bg-gray-100"
                      }`}
                  >
                    {link.name}
                  </Link>
